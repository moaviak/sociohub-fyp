...
/**
 * Checks if the user have event_ticket_handling privilege
 * @param userId - User id
 * @param societyId - id of the society
 * @returns A URL string for the avatar
 */
export const haveTicketHandlingPrivilege = async (
  userId: string,
  societyId: string
) => {
  // Check if the user is the advisor of the society
  ...
  const studentWithPrivilege = await prisma.studentSocietyRole.findFirst({
    where: {
      ...
      role: {
        privileges: {
          some: { key: "event_ticket_handling" },
        },
      },
    },
    select: { studentId: true },
 ...
 * @returns A URL string for the avatar
 */
export const haveAnnouncementsPrivilege = async (
  userId: string,
  societyId: string
) => {
  // Check if the user is the advisor of the society
  ...
  const studentWithPrivilege = await prisma.studentSocietyRole.findFirst({
    where: {
      ...
      role: {
        privileges: {
          some: { key: "announcement_management" },
        },
      },
    },
    select: { studentId: true },
 ...
 * @returns A URL string for the avatar
 */
export const haveMeetingsPrivilege = async (
  userId: string,
  societyId: string
) => {
  // Check if the user is the advisor of the society
 ...
 * @returns A URL string for the avatar
 */
export const havePaymentsPrivilege = async (
  userId: string,
  societyId: string
) => {
  // Check if the user is the advisor of the society
  ...
  const studentWithPrivilege = await prisma.studentSocietyRole.findFirst({
    where: {
      ...
      role: {
        privileges: {
          some: { key: "payment_finance_management" },
        },
      },
    },
    select: { studentId: true },
...
/**
 * Checks if the user have teams_management privilege
 * @param userId - User id
 * @param societyId - id of the society
 * @returns A URL string for the avatar
 */
export const haveTeamsPrivlege = async (userId: string, societyId: string) => {
  // Check if the user is the advisor of the society
  const society = await prisma.society.findUnique({
    where: { id: societyId },
    select: { advisor: { select: { id: true } } },
 ...
 * @returns A URL string for the avatar
 */
export const haveTaskPrivilege = async (userId: string, societyId: string) => {
  // Check if the user is the advisor of the society
  const society = await prisma.society.findUnique({
    where: { id: societyId },
    select: { advisor: { select: { id: true } } },
